...
int duty_turnon = 2100;	//70% duty cycle to turn on
...
int main(void){
    ...
    while(1){
    	...
    	if(button1==pressed){
    	    ...
    	    while (time<10){}	//waits 2 seconds before beginning to move

    	    //have car start moving using 70% duty cycle, then slow down to pre-defined duty cycle
    	    motor1rev = 0;
    	    motor2rev = 0;
    	    motor1for = duty_turnon;
    		motor2for = duty_turnon;
    		time = 0;
    	    while (time<2){}
    		motor1for = duty;
    		motor2for = duty;
