...
float tempout = 0;
float allMeas[30] = {};
float *output_from_flash_float[30];
char *output_from_flash_char[120];
    		...
    		if (but2 == 0){
    			...
    			//data output routine:
    				...
    				while((UCA0IFG & 0x02) == 0){} ;
       				...
       				for(i=0; i<120; i++){
    					output_from_flash_char[i] = (char*)MAIN_1_SECTOR_31 + i;
      					char out1[4] = {};
       					sprintf(out1,"%s",output_from_flash_char[i]);

       					for (j=0;j<4;j++){
       						MAP_UART_transmitData(EUSCI_A0_MODULE, out1[j]); //send each data byte
       					}
...
void timer_a_0_isr(void)
{
	if (inputcount<totalMeasurements){
    	...
    	sprintf(string_from_flash,"%2.1f", tempout);
    	for(i=0;i<4;i++){
    	toprint[numchars+i] = string_from_flash[i];
    	}
		...
		numchars = totalMeasurements * 4 ;		//each temp value will contain 4 chars ("XX.X")
		MAP_FlashCtl_unprotectSector(FLASH_MAIN_MEMORY_SPACE_BANK1, FLASH_SECTOR31);		//unprotect sector
		if(!MAP_FlashCtl_eraseSector(MAIN_1_SECTOR_31)){};									//erase current data
		//if(!MAP_FlashCtl_programMemory(allMeas, (void*) MAIN_1_SECTOR_31, totalMeasurements)){};			//program data
		if(!MAP_FlashCtl_programMemory(toprint, (void*) MAIN_1_SECTOR_31, 120)){};			//program data
		MAP_FlashCtl_protectSector(FLASH_MAIN_MEMORY_SPACE_BANK1, FLASH_SECTOR31);			//protect the sector
